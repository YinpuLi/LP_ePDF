...
par2val             = function(pars){
    val_a           = pars[1]
    val_M           = val_a + exp(pars[2])
    val_b           = val_M + exp(pars[3])
    return(c(val_a, val_M, val_b))
}

val2par             = function(vals){
    par1            = vals[1]
    par2            = log(vals[2] - vals[1])
    par3            = log(vals[3] - vals[2])
...
validate_val_vec    = function(
    ...
    if(
        valid_val_vec[3] <= Min
    ){  valid_val_vec     = c(Min + 0.01, Min + 0.02, Min + 0.03)
    }
    if(
        valid_val_vec[2] <= Min
    ){
        valid_val_vec[2]  = min(Min + 0.01,
                                median(c(valid_val_vec[3], Min)))
    ...
    if(
        ...
        valid_val_vec     = c(Max - 0.03, Max - 0.02, Max - 0.01)
    }
    if(
        valid_val_vec[2] >= Max
    ){
        ...
        valid_val_vec[3]  = max(Max - 0.01,
                                median(c(valid_val_vec[2], Max)))
    }
    if(
        valid_val_vec[3] >= Max
    ){
