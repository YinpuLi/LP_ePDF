...
for(i in 12:20
    ...
    result_list = list(
        IMPR_max = IMPR_max,
        IMPR_max_copy = IMPR_max_copy,
        TIME_max = TIME_max,
        best_par = fit_temp$par,
        best_val = old_mthd_val,
        solve_res= solveLP_max_min(new_pars = fit_temp$par,
                                   new_t    = my_interest[i])
    )
    
    capture.output(result_list, file = paste0("result", my_interest[i],".txt"))
